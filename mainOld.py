...
def main():
    file_path = 'large-test-dataset-1.txt'
    X, Y = load_data(file_path)
    X = normalize_features(X)
    knn = KNNClassifier(k=3)

    validator = Validator(knn)
    feature_subset = [0, 14, 26] #We ignore 1st column so feature 3 should be in column 2 
    #So if we wanted to test features 1, 15, 27 we put [0,14,26]
